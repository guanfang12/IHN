...
class homo_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            perturbed_four_points_cord = [] # 扰动后的四个角点
            for i in range(4):
            ...
            point = np.vstack((x_grid.flatten(), y_grid.flatten())).transpose() # [h*w, 2]
        ...
        # 这样的话就是 H 可以把warped_image 变为img2
        ...
        diff_branch1 = point_transformed_branch1 - np.array(point, dtype=np.float64) # 光流
        ...
        img_patch_ori = img_patch_ori[:, :, ::-1].copy() # BGR 2 RGB 
        ...
        flow = torch.from_numpy(pf_patch).permute(2, 0, 1).float() #[x, y]
        ...
        four_point[:, 0, 0] = flow[:, 0, 0] + torch.Tensor([0, 0]) # LT
        four_point[:, 0, 1] = flow[:, 0, -1] + torch.Tensor([128 - 1, 0]) # RT
        four_point[:, 1, 0] = flow[:, -1, 0] + torch.Tensor([0, 128 - 1]) # LB
        four_point[:, 1, 1] = flow[:, -1, -1] + torch.Tensor([128 - 1, 128 - 1]) # RB
...
class MYDATA(homo_dataset):
    def __init__(self, split='train', dataset='mscoco'):
        super(MYDATA, self).__init__()
        if split == 'train':
            if dataset=='mscoco':
                root_img1 = '/home/wxl/Data/COCO2014/train2014'
                root_img2 = '/home/wxl/Data/COCO2014/train2014'
            if dataset=='ggmap':
            ...
            if dataset=='mscoco':
                root_img1 = '/home/wxl/Data/COCO2014/test2014'
                root_img2 ='/home/wxl/Data/COCO2014/test2014'  # './mscoco2017/test2017'
            if dataset=='ggmap':
